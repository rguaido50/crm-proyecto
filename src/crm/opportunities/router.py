...
from crm.opportunities.models import Opportunity, OpportunityStage
...
@router.get("", response_model=dict[OpportunityStage, list[OpportunityRead]])
async def list_open_by_stage(
    session: AsyncSession = Depends(get_session),
) -> dict[OpportunityStage, list[Opportunity]]:
    return await service.list_open_by_stage(session)
